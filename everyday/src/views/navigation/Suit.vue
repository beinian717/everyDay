<template>
  <div class="suit_container">
    <!-- 头部部分 -->
    <div class="suit_top_container">
      <div class="top_container">
        <div @click="backClick">
          <img
            style="width:1rem;height:1.5rem;"
            src="data:image/png;base64,"
          />
        </div>
        <div style="font-size:1.3rem;color:#595959">{{this.$route.meta.title}}</div>
      </div>
      <div style="width:30%;height:100%;display:flex;justify-content:flex-end;align-items:center;">
        <img
          style="width:40px;height:40px;"
          src="data:image/png;base64,"
        />
      </div>
    </div>
    <!-- 选择部分 -->
    <div class="suit_select_container">
      <van-dropdown-menu active-color="orange" style="width:100%;">
        <van-dropdown-item title="套卷分类" >
          <div class="confirm_container">
            <div class="confirm_left">
                <div :class="leftSelIndex==index?'confirm_left_item_active':'confirm_left_item'" @click="leftClick(item,index)" v-for="(item,index) in left1" :key="index">
                  {{item.name}}
                </div>
            </div>
            <div class="confirm_right1">
              <div class="confirm_right1_item" v-for="(item,index) in temp" :key="index">
                {{item.name}}
              </div>
            </div>
          </div>
        </van-dropdown-item>
        <van-dropdown-item title="套卷状态" ref="item">
         <div class="confirm_Right">
           <div @click="RightClick(index)" :class="RightSelIndex==index?'confirm_Right_item_active':'confirm_Right_item'" v-for="(item,index) in Right" :key="index">
             {{item}}
           </div>
         </div>
        </van-dropdown-item>
      </van-dropdown-menu>
    </div>
    <!-- 提示部分 -->
    <div class="suit_logo_container">
      <img src="../../assets/empty.png" alt />
      <p>请稍候，套卷正在赶来的路上啦</p>
    </div>
  </div>
</template>

<script>
import axios from 'axios';

export default {
  components: {},
  data() {
    return {
      left1:[],
      right1:[],
      Right:[],
      temp:[],
      leftSelIndex:0,
      RightSelIndex:0,
    };
  },
  computed: {},
  methods: {
    leftClick(item,index){
      this.leftSelIndex=index;
      this.temp=[];
      this.right1.forEach((ele)=>{
        if(ele.pid==item.id){
          this.temp.push(item);
        }
      })
    },
    RightClick(index){
      this.RightSelIndex=index;
    },
    backClick() {
      window.history.back();
    }
  },
  mounted() {
    axios.get("/suit.json").then((resp)=>{
      this.left1=resp.data.left1;
      this.right1=resp.data.right1;
      this.Right=resp.data.Right;
    })
  }
};
</script>
<style  scoped>
.van-dropdown-menu__title{
  font-size:20px;
}
.suit_container {
  width: 100%;
}
.suit_top_container {
  width: 100%;
  height: 4rem;
  display: flex;
  justify-content: flex-start;
  align-items: center;
}
.top_container {
  width: 59%;
  height: 100%;
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-left: 0.4rem;
}
.suit_select_container {
  width: 100%;
  height: 80px;
  display: flex;
  justify-content: flex-start;
  align-items: center;
}
.confirm_container{
  width:100%;
  height:300px;
  background:#fff;
  display: flex;
}
.confirm_left{
  width:157px;
  height:100%;
  display: flex;
  flex-direction: column;
  justify-content: flex-start;
  align-items:center;
}
.confirm_left_item{
  width:100%;
  height:60px;
  display: flex;
  justify-content: center;
  align-items:center;
  color:#595959;
}
.confirm_left_item_active{
  width:100%;
  height:60px;
  display: flex;
  justify-content: center;
  align-items:center;
  color:orange;
}
.confirm_right1{
  width:236px;
  height:100%;
  display: flex;
  flex-direction: column;
  justify-content: flex-start;
  align-items:center;
}
.confirm_right1_item{
    width:100%;
    height:100%;
    color:#595959;
    padding:15px 15px;
    box-sizing: border-box;
}
.confirm_Right{
  width:100%;
  height:250px;
  background:#fff;
}
.confirm_Right_item{
  width:100%;
  height:80px;
  display: flex;
  justify-content: center;
  align-items:center;
  color:#595959;
}
.confirm_Right_item_active{
  width:100%;
  height:80px;
  display: flex;
  justify-content: center;
  align-items:center;
  color:orange;
}
.suit_logo_container {
  width: 100%;
  height: 280px;
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  margin-top: 10px;
}
</style>